...
const Services = () => {
  ...
  const filteredServices = useMemo(() => {
    ...
    if (state.selectedCategory) {
      return state.services
        .filter(
          (category) => category && category.name === state.selectedCategory
        )
        .map((category) => ({
          ...category,
  ...
  const fetchServices = useCallback(async () => {
    try {
      const response = await axios.get(
        ...
        }/backend/api/public_services.php?action=services`
      );
  ...
  const fetchZones = useCallback(async () => {
    try {
      const response = await axios.get(
        `${
          import.meta.env.VITE_API_URL
        }/backend/api/admin/locations.php?type=zones`
      );
      if (response.data.success) {
  ...
  const fetchAreas = useCallback(async (zoneId) => {
    ...
    try {
      const response = await axios.get(
        `${
          import.meta.env.VITE_API_URL
        }/backend/api/admin/locations.php?type=areas`
      );
      if (response.data.success) {
        // Filter areas by zone_id
        const filteredAreas = response.data.data.filter(
          (area) => area.zone_id == zoneId
        );
  ...
  const fetchServiceDetails = useCallback(async (serviceId) => {
    try {
      const response = await axios.get(
        `${
          import.meta.env.VITE_API_URL
        }/backend/api/public_services.php?action=service&id=${serviceId}`
      );
      if (response.data.success) {
        setState((prev) => ({
          ...prev,
  ...
  const handleBookingSubmit = useCallback(
    async (e) => {
      ...
      const { name, email, phone, address, preferred_date, zone_id, area_id } =
        state.bookingData;
      if (
        !name ||
        !email ||
        ...
        toast.error(
          "Please fill in all required fields including zone and area"
        );
        return;
      }
      ...
      try {
        const bookingPayload = {
          ...
          description:
            ...
            `Service request for ${state.selectedService.name}`,
          ...
          scheduled_at: `${state.bookingData.preferred_date} ${
            state.bookingData.preferred_time || "09:00"
          }`,
          urgency: "normal",
        };
        ...
        const response = await axios.post(
          `${
            import.meta.env.VITE_API_URL
          }/backend/api/user/request_service.php`,
          bookingPayload,
        ...
        if (response.data.success) {
          ...
          toast.error(
            response.data.message || "Failed to submit service request"
          );
        }
      } catch (error) {
    ...
    >
      <Container className="py-3">
                ...
                title={
                  <span
                    style={{
                      ...
                      backgroundColor:
                        state.activeTab === "all" ? "black" : "transparent",
                      padding: "4px 8px",
                      borderRadius: "4px",
                ...
                title={
                  <span
                    style={{
                      ...
                      backgroundColor:
                        state.activeTab === "popular" ? "black" : "transparent",
                      padding: "4px 8px",
                      borderRadius: "4px",
                      display: "inline-block",
                    }}
              ...
              <Tab
                ...
                title={
                  <span
                    style={{
                      color:
                        state.activeTab === "emergency" ? "white" : "black",
                      backgroundColor:
                        ...
                        >
                          <Card className="h-100 shadow-sm border-0">
                            {service.image && (
                              <Card.Img
                                variant="top"
                                src={`${import.meta.env.VITE_API_URL}/backend/${
                                  service.image
                                }`}
                                alt={service.name}
                                style={{
          ...
          <Row className="align-items-center mb-5">
            <Col lg={6}>
              ...
              <p className="lead mb-4">
                We deliver exceptional service with certified professionals,
            ...
            {state.selectedService && (
              <>
                <div className="mb-3">
                  ...
                  {state.selectedService.category_name}
                </div>
                <div className="mb-3">
                  <strong>Price:</strong> ${state.selectedService.base_price}/
                ...
                <div className="mb-3">
                  ...
                  <p className="mt-2">
                    {state.selectedService.description ||
                      "Professional service with quality guarantee and certified technicians."}
                  </p>
                </div>
                <div className="mb-3">
        ...
        <Modal
          ...
          onHide={() =>
            setState((prev) => ({ ...prev, showBookingModal: false }))
          }
          size="lg"
              ...
              <Row>
                    ...
                    <DatePicker
                      selected={
                        state.bookingData.preferred_date
                          ? new Date(state.bookingData.preferred_date)
                          : null
                      }
                      onChange={(date) =>
                        setState((prev) => ({
                ...
                <Col md={3}>
                  <Form.Group className="mb-3">
                    ...
                    <DatePicker
                      selected={
                        state.bookingData.preferred_time
                          ? new Date(
                              `1970-01-01T${state.bookingData.preferred_time}`
                            )
                          : null
                      }
                      onChange={(time) =>
                        setState((prev) => ({
                          ...
                          bookingData: {
                            ...prev.bookingData,
                            preferred_time: time.toLocaleTimeString([], {
                              hour: "2-digit",
                              minute: "2-digit",
                            }),
                          },
              ...
              {state.selectedService && (
                <div className="bg-light p-3 rounded">
                  ...
                  <p className="mb-0 text-muted">
                    Starting from ${state.selectedService.base_price}/
                    {state.selectedService.unit}
                  </p>
                </div>
              )}
        ...
        <Modal
          ...
          onHide={() =>
            setState((prev) => ({ ...prev, showLoginPromptModal: false }))
          }
          centered
          aria-labelledby="login-prompt-modal-title"
        >
          <Modal.Header closeButton>
            <Modal.Title id="login-prompt-modal-title">
              Login Required
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
